<template>
  <div>
    <div class="table-label">Previous Board Meeting</div>
    <div class="event-table">
        ...
        <table class="event-table__table">
          <thead>
            <tr class="event-table__header">
              ...
              <th class="event-table__cell">Actions</th> <!-- Action Column -->
            </tr>
          </thead>
          <tbody>
            <tr v-for="event in events" :key="event.id">
              <td class="event-table__cell event-table__title">{{ event.title }}</td> <!-- Left-aligned Title -->
              <td class="event-table__cell">
                <button 
                  @click="openModal(event)" 
                  class="event-table__button">View Details</button> <!-- Action Button -->
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <EventModal v-if="isModalVisible" :event="selectedEvent" @close="closeModal" @download="downloadCalendar"/>
    </div>
  </div>
</template>
